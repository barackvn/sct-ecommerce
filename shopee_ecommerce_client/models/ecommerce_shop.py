...
#        _logger.info(self.name)
    ...
    def _sync_product_sku_match_shopee(self):
        self.ensure_one()
        platf_id = self.env['ecommerce.platform'].search([('platform', '=','shopee')])[:1].id
        resp = self._py_client_shopee().item.get_item_list(pagination_offset=0,pagination_entries_per_page=100)
        ...
        for item in items:
            ...
            if item.get("variations",[]): 
                ...
                tmpls = self.env['product.template'].search([('default_code','=',item.get('item_sku','').strip())])
                if tmpls: 
                    details = self._py_client_shopee().item.get_item_detail(item_id=item.get('item_id',0)).get('item',{})
                for tmpl in tmpls:
                    if item_idn not in tmpl.shopee_product_tmpl_ids.mapped('platf_item_idn'):
