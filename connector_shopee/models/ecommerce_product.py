...
class ShopeeProductTemplate(models.Model):
    ...
    def _update_stock_shopee(self):
        self.ensure_one()
        if not self.ecomm_product_product_ids:
            stock = int(self.product_product_id.virtual_available) if (self.product_tmpl_id.type == 'product' and self.product_tmpl_id.inventory_availability not in [False, 'never']) else 1000
            self.shop_id._py_client_shopee().item.update_stock(item_id = int(self.platf_item_idn), stock = stock)
        else:
            stock_values = [{
                'variation_id': int(v.platf_variant_idn),
                'stock': int(v.product_product_id.virtual_available) if (self.product_tmpl_id.type == 'product' and self.product_tmpl_id.inventory_availability not in [False, 'never']) else 1000,
                'item_id': int(self.platf_item_idn),
                } for v in self.ecomm_product_product_ids]
            self.shop_id._py_client_shopee().item.update_variation_stock_batch(variations = stock_values)
...
class ShopeeProductProduct(models.Model):
    ...
    def _update_variantion_stock_shopee(self):
        self.ensure_one()
        stock = int(self.product_product_id.virtual_available) if (self.product_product_id.product_tmpl_id.type == 'product' and self.product_product_id.product_tmpl_id.inventory_availability not in [False, 'never']) else 1000
        self.shop_id._py_client_shopee().item.update_variation_stock(item_id=int(self.ecomm_product_tmpl_id.platf_item_idn), variation_id= int(self.platf_variant_idn), stock=stock)
