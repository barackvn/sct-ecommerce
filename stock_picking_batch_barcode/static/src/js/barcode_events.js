...
var BarcodeEvents = core.Class.extend(mixins.PropertiesMixin, {
    ...
    _handleBarcodeValue: function (e) {
        ...
        self.waitAlert('[name="alert"]', function() {
            if ($('.alert').not('.o_invisible_modifier')) {
                var sound = $('.alert').not('.o_invisible_modifier')[0].className.split(/\s+/).find(function (c){
                    return c.startsWith('alert-');
                }).slice(6);
    ...
    play_sound: function(sound) {
        var src='';
        if (sound === 'danger') {
            src = "/stock_picking_batch_barcode/static/src/sounds/error.wav";
        } else if (sound === 'success') {
            src = "/stock_picking_batch_barcode/static/src/sounds/bell.wav";
        } else {
            //window.console.error('Unknown sound: ',sound);
            return;
        }
        $('body').append('<audio src="'+src+'" autoplay="true"></audio>');
    },

    waitAlert: function (selector, callback, max = false) {
        var self = this;
            if ($(selector).length && $(selector)[0].innerHTML == "1") callback();
            else {
                if (max === false || max > 0) {
                    max != false && max--;
                    setTimeout(function() {self.waitAlert(selector, callback, max);}, 100);
                }
            }
    },
...
/*odoo.define("barcodes.BarcodeEvents", function(require) {
    ...
    BarcodeEvents.BarcodeEvents.__proto__.play_sound = function(sound) {
        var src='';
        if (sound === 'danger') {
            src = "/stock_picking_batch_barcode/static/src/sounds/error.wav";
        } else if (sound === 'success') {
    ...
    //BarcodeEvents.BarcodeEvents.__proto__.alertTimeout = null;
    BarcodeEvents.BarcodeEvents.__proto__.waitAlert = function (selector, callback, max = false) {
        var self = this;
            if ($(selector).length && $(selector)[0].innerHTML == "1") callback();
            else {
                if (max === false || max > 0) {
    ...
    var _super_ = BarcodeEvents.BarcodeEvents.__proto__._handleBarcodeValue;
    BarcodeEvents.BarcodeEvents.__proto__._handleBarcodeValue = function(e) {
        var self = this;
        //clearTimeout(this.alertTimeout);
        //$("[name='alert']")[0].innerHTML = "0";
        _super_.apply(this, arguments);

        console.log(123);
        if ($.isReady) self.waitAlert('[name="alert"]', function() {
            if ($('.alert').not('.o_invisible_modifier')) {
                var sound = $('.alert').not('.o_invisible_modifier')[0].className.split(/\s+/).find(function (c){
                    return c.startsWith('alert-');
                }).slice(6);  
