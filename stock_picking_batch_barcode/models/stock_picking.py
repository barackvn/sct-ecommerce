...
class StockPicking(models.Model):
    ...
    def _compute_all_quantities_done(self):
        for pick in self:
            ...
            if all(float_compare(move.quantity_done,move.product_uom_qty,precision_digits=precision_digits) == 0 for move in pick.move_lines.filtered(lambda m: m.state not in ('done', 'cancel'))):
                pick.message_post(body='Logistic time stamp: {}'.format(fields.Datetime.now()))
